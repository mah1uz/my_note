 ...
 */
export function useTaskCompletion(item, onChanged) {
...
function ExpenseRow({ item, onChanged }) {
  ...
  return <article className={`structured-card${done ? ' completed' : ''}`}>
    <div className="task-row event-row">
      <button type="button" className="check-button" aria-pressed={done} disabled={completion.busy} aria-label={`Mark ${item.title} ${done ? 'accounted' : 'open'}`} onClick={toggle}>{done ? '✓' : ''}</button>
    ...
    <div className="domain-list">{item.domains.map((domain) => <span className="pill" key={domain}>{domain}</span>)}</div>
    {item.summary && <p>{item.summary}</p>}
    <p><strong>{item.amount == null ? 'Amount not specified' : `${item.currency || 'Currency unknown'} ${item.amount}`}</strong>{item.quantity != null && <> {item.quantity} {item.unit || 'unit unknown'}</>}</p>
